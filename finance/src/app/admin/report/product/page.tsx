...
export default function ProductReportPage() {
  ...
  useEffect(() => {
    if (user !== null && isCustomer && user?.id) {
      setApplied((prev) => ({ ...prev, merchantId: user.id }));
    }
  }, [user, isCustomer]);

  const { data: deliveriesWithItems = [], isFetching: loading } = useQuery({
    queryKey: queryKeys.productReport(applied),
    queryFn: async (): Promise<DeliveryWithItems[]> => {
      const deliveries = await fetchReportDeliveriesWithItems({
        startDate: applied.startDate,
        endDate: applied.endDate,
        merchantId: applied.merchantId,
      });
      const rows: DeliveryWithItems[] = [];
      for (const delivery of deliveries) {
        const items: DeliveryItem[] = delivery.items || [];
        let filteredItems = items;
        if (applied.productName.trim()) {
          filteredItems = items.filter((item) =>
            item.good?.name?.toLowerCase().includes(applied.productName.toLowerCase())
          );
        }
        if (filteredItems.length > 0 || !applied.productName.trim()) {
          rows.push({ ...delivery, items: filteredItems });
        }
      }
      return rows;
    },
  ...
  const handleSubmit = () => {
    ...
    setApplied({
      ...
      merchantId: isCustomer && user?.id ? user.id : selectedMerchantId ?? undefined,
