...
// Driver status breakdown report
export const fetchDriverStatusReport = async (filters: {
  startDate: string;
  endDate: string;
  driverId?: number;
}): Promise<DriverStatusReportResult> => {
  if (!API_URL) {
    throw new Error('API URL is not configured. Please set NEXT_PUBLIC_API_URL environment variable.');
  }
  ...
  if (filters.driverId) {
    params.append('driver_id', filters.driverId.toString());
  }

  const response = await fetch(`${API_URL}/api/report/driver-stats?${params.toString()}`, {
    headers: getAuthHeaders(),
  });
  ...
  throw new Error(result.message || 'Failed to fetch driver status report');
};
...
export const fetchProductReport = async (
  ...
  try {
    const merchantsResponse = await fetch(`${API_URL}/api/user/merchant?include_inactive=1`, {
      headers: getAuthHeaders(),
    });
