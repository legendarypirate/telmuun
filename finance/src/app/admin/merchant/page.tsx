...
export default function MergeReportPage() {
  ...
  const handlePriceChange = async (id: number, newPrice: string) => {
    if (isMerchant) {
      toast.error("Merchant users cannot edit prices");
      return;
    }
  ...
  const handleDeliveryPrice = async (id: number, newPrice: string) => {
    if (isMerchant) {
      toast.error("Merchant users cannot edit delivery prices");
      return;
    }

    try {
      setData((prev) => prev.map((item) => (item.id === id ? { ...item, delivery_price: newPrice } : item)));
  ...
  const handleMergeReport = async () => {
    ...
    const totalDeliveryPriceVal = selectedRows.reduce((sum, item) => sum + parseFloat(item.delivery_price || "0"), 0);
    const netPriceVal = totalPriceVal - totalDeliveryPriceVal;
  ...
  const totalDeliveryPrice = selectedItems.reduce((sum, item) => sum + parseFloat(item.delivery_price || "0"), 0);
  const netPrice = totalPrice - totalDeliveryPrice;
  const paged = data.slice((page - 1) * pageSize, page * pageSize);

  return (
    <div className="pb-24 p-5">
      <h4 className="mb-4 text-xl font-bold">Дэлгүүрийн тайлан</h4>
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead>Хүргэлтийн үнэ</TableHead>
            ...
            <TableBody>
              {paged.map((record) => (
                <TableRow key={record.id}>
                  ...
                  <TableCell>
                    {isMerchant ? (
                      <span>{record.delivery_price}</span>
                    ) : (
                      <Input type="number" className="w-[100px]" value={record.delivery_price} onChange={(e) => handleDeliveryPrice(record.id, e.target.value)} />
                    )}
                  </TableCell>
      ...
      <div className="fixed bottom-0 left-64 right-0 z-[999] flex flex-wrap items-center gap-4 border-t bg-background p-4">
        ...
        <span>🚚 Total Delivery Price: <b>{totalDeliveryPrice.toLocaleString()}₮</b></span>
        <span>🧾 Net Price: <b>{netPrice.toLocaleString()}₮</b></span>
        {!isMerchant && <Button onClick={handleMergeReport}>Тайлан нийлэх</Button>}
        <Button
          variant="outline"
