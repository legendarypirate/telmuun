import { Good, Merchant, Ware, CreateGoodPayload, UpdateStockPayload, UpdateGoodPayload, GoodHistory } from '../types/good';
...
export const fetchMerchants = async (): Promise<Merchant[]> => {
  ...
  if (result.success) {
    ...
    return users.filter((u: any) => u && u.role_id === 2 && u.id != null);
  }
...
// Update good (name, ware, etc.)
export const updateGood = async (
  goodId: number,
  payload: UpdateGoodPayload
): Promise<Good> => {
  ...
  throw new Error(result.message || 'Failed to update good');
};
